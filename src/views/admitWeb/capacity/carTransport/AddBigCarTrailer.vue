                ...
                <el-form v-if="activePop === 0" :model="addHeadstockForm" :rules="addHeadstockRules"
                    ...
                    <div class="info-item clear">
                        <el-form-item label="挂车牌号" prop="dlPlateNo">
                            <el-input type="text" v-model="addHeadstockForm.dlPlateNo"></el-input>
                        </el-form-item>
                    ...
                    <div class="info-item clear" v-if="addHeadstockForm.isCtali === 1">
                        <el-form-item label="交强险保险品牌" prop="ctaliType">
                            <el-select v-model="addHeadstockForm.ctaliType" placeholder="请选择"  @change="getCtaliType(addHeadstockForm.ctaliType)">
                    ...
                    <div class="info-item clear" v-if="addHeadstockForm.isCi  === 1">
                        <el-form-item label="商业险保险品牌" prop="ciType">
                            <el-select v-model="addHeadstockForm.ciType" placeholder="请选择" @change="getCiType(addHeadstockForm.ciType)">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            /*getCtaliType(val){
                if(val == '0'){
                    this.addHeadstockForm.isCtali=0;
                    this.addHeadstockForm.ctaliType=''
                }
            },
            getCiType(val){
                if(val == '0'){
                    this.addHeadstockForm.isCtali=0;
                    this.addHeadstockForm.ciType = '';
                    this.addHeadstockForm.isCi = 0;
                }
            },*/
            closeModal: function () {
                // this.reload();
                ...
                this.$parent.initCarTrailerPkId();
            },
            /*************图片处理----start************/
            handleAvatarSuccess(res, file) {
                ...
                if (attrs) {
                    if ("driving_license" === imageType) { //行驶证正面
                        this.addHeadstockForm.dlPlateNo = attrs['vehiclePlateNumber'] == null ? '' : attrs['vehiclePlateNumber'];          // 车牌号码
                        this.addHeadstockForm.dlCarType = attrs['vehicleType'] == null ? '' : attrs['vehicleType'];                 // 车辆类型
                        this.addHeadstockForm.dlOwner = attrs['vehicleOwner'] == null ? '' : attrs['vehicleOwner'];                  // 所有人
                        ...
                        this.addHeadstockForm.dlHostlingWeight = attrs['vehicleCurbWeight'] == null ? '' : attrs['vehicleCurbWeight'];                // 整备质量
                ...
                if (this.pkId) {
                    ...
                    that.$get("ltdCarTrailer/selectOne", {"pkId": that.pkId}).then(function (response) {
                        if (response.response != null) {
                            ...
                            if(response.response.ltdCarTrailer.isCtali==0){
                                that.addHeadstockForm.ctaliType = '';
                            }
                            if(response.response.ltdCarTrailer.isCi==0){
                                that.addHeadstockForm.ciType = '';
                            }
                            that.addHeadstockForm.dlCheckEndTime=that.common.formatTimestamp(response.response.ltdCarTrailer.dlCheckEndTime)
                            that.addHeadstockForm.dlRegisterTime=that.common.formatTimestamp(response.response.ltdCarTrailer.dlRegisterTime)
                            that.addHeadstockForm.dlIssueTime=that.common.formatTimestamp(response.response.ltdCarTrailer.dlIssueTime)
                            that.addHeadstockForm.dlScrapEndTime=that.common.formatTimestamp(response.response.ltdCarTrailer.dlScrapEndTime)
                            if(response.response.ltdCarTrailer.verifyStatus == 2){

                                that.showErrorInfo=response.response.ltdCarTrailer.verifyReason;
                            }
