<template>
	<div class="case">
					...
					<div class="car-way no-choice-box">
						...
						<div class="car-tabs list-search-box">
								...
								<div class="search-manage small-search small-car-search clear">
									<div class="search-l clear">
										<el-form-item label="司机姓名">
											<el-input placeholder="请输入司机姓名" v-model="searchUserName"></el-input>
										</el-form-item>
										<el-form-item label="驾驶证号">
											<el-input placeholder="请输入驾驶证号" v-model="searchDLNo"></el-input>
										</el-form-item>
										<el-form-item label="从业资格证号" label-width="110px">
											<el-input placeholder="请输入从业资格证号" v-model="searchQcNo"></el-input>
										</el-form-item>
										<el-form-item label="审核状态">
											<el-select v-model="searchAuthState">
												<el-option
														v-for="item in searchVerifyOp"
														:key="item.id"
														:label="item.name"
														:value="item.id">
												</el-option>
										...
										<el-form-item class="search-date" label="审核时间" prop="">
											<el-date-picker type="date" v-model="searchVerifyStartTime" placeholder="开始日期">
											</el-date-picker>
											<span>至</span>
											<el-date-picker type="date" v-model="searchVerifyEndTime" placeholder="结束日期">
											</el-date-picker>
										</el-form-item>
										<el-form-item class="search-date" label="更新时间" prop="">
											<el-date-picker type="date" v-model="searchUpdateStartTime" placeholder="开始日期">
											</el-date-picker>
											<span>至</span>
											<el-date-picker type="date" v-model="searchUpdateEndTime" placeholder="结束日期">
											</el-date-picker>
										</el-form-item>
										<el-form-item label="司机类型">
											<el-select v-model="searchDriverType">
												<el-option
														v-for="item in searchDriverTypeOp"
														:key="item.id"
														:label="item.name"
														:value="item.id">
												</el-option>
										...
										<el-form-item label="关联车辆">
											<el-select v-model="searchBindType">
												<el-option
														v-for="item in searchBindTypeOp"
														:key="item.id"
														:label="item.name"
														:value="item.id">
												</el-option>
							...
							<div class="car-way-nothing" v-if="!tableData.length">
								<div class="car-way-no" style="top:280px;">
							...
							<div class="car-way-table">
								<el-table
									...
									<el-table-column
											...
											width="150">
										<template slot-scope="scope">
											<span>{{scope.row.driverType==1?"大板司机":"小板司机"}}</span>
										</template>
									</el-table-column>
									<el-table-column
											prop="dlNo"
									...
									<el-table-column
											...
											width="150">
										<template slot-scope="scope">
											<a @click="qcDetailsInfoShow(scope.row.pkId)" class="car-way-blue" href="javascript:;">{{scope.row.qcNo}}</a>
										</template>
									</el-table-column>
									<el-table-column
											prop="isBindCar"
											...
											width="150">
										<template slot-scope="scope">
											<span>{{scope.row.isBindCar==0?"未关联":"已关联"}}</span>
										</template>
									</el-table-column>
									<el-table-column
											prop="authState"
									...
									<el-table-column
											...
											width="170">
										<template slot-scope="scope">
											<span>{{scope.row.modifyTime==null?common.formatterDateTime('','',scope.row.createTime):common.formatterDateTime('','',scope.row.modifyTime)}}</span>
										</template>
									</el-table-column>
									<el-table-column fixed="right" label="操作" width="220">
										<template slot-scope="scope">
											...
											<el-button type="text" size="small" v-if="scope.row.verifyStatus==1 || (scope.row.verifyStatus==0 && scope.row.isUpPic==1)"
													   @click="updateInfoFun(scope.row.pkId)">编辑
											</el-button>
											<el-button type="text" size="small" v-if=" scope.row.verifyStatus==2"
													   @click="updateInfoFun(scope.row.pkId)">重新提交
											</el-button>
											<el-button type="text" size="small" v-if=" scope.row.verifyStatus!=3"
													   @click="delConfir(scope.row.pkId,scope.row.driverType)">解雇司机
											</el-button>
											<el-button type="text" size="small" v-if=" scope.row.verifyStatus==1 && scope.row.isBindCar ==0"
			...
			<div class="capacity-pop-in">
				<div class="steps steps-step">
					<el-steps :active="activePop" finish-status="success" process-status="finish" align-center>
						...
						<el-step title="添加成功提交审核"></el-step>
					</el-steps>
				</div>
			...
			<div class="capacity-pop-in info-case" style="padding: 0;">
				...
				<el-form :model="updateForm" :rules="updateRules" ref="updateForm" label-width="130px" class="demo-ruleForm">
					...
					<div class="info-item clear">
						...
						<el-form-item label="联系地址" class="address" prop="address">
		...
		<!--上传司机证件弹窗-->
		<el-dialog class="capacity-pop" title="上传司机证件" :visible.sync="uploadPapers" width="640px" center>
			<div class="capacity-pop-in">
				<el-form :model="uploadPapersForm" :rules="uploadPapersRules" ref="uploadPapersForm" label-width="80px"
						 class="demo-ruleForm papers-item">
					<div class="upload_box info-item">
		...
		<!--核对并完善信息弹窗-->
		<el-dialog class="capacity-pop capacity-pop-big" title="添加核对并完善信息" :visible.sync="checkInfo" width="74.2%" center>
			<div class="capacity-pop-in info-case" style="padding: 0;">
				...
				<el-form :model="checkInfoForm" :rules="checkInfoRules" ref="checkInfoForm" label-width="110px"
					...
					<div class="info-item clear">
						...
						<el-form-item label="备用电话" prop="">
							<el-input v-model="checkInfoForm.backupPhone" maxlength="13"></el-input>
						</el-form-item>
						...
						<el-form-item label="联系地址" class="address required_flag" prop="address">
							...
							<el-input v-model="checkInfoForm.address" class="width-other" prop="address">
		...
		<el-dialog class="capacity-pop capacity-pop-big" title="上传图片核对并完善信息" :visible.sync="upCheckInfo" width="74.2%" center>
			<div class="capacity-pop-in info-case" style="padding: 0;">
				<p class="checkInfo-tip">已为您自动识别部分信息，请仔细核对并完善</p>
				<el-form :model="upCheckInfoForm" :rules="upCheckInfoRules" ref="upCheckInfoForm" label-width="110px"
					...
					<div class="info-item clear">
						...
						<el-form-item label="备用电话" prop="">
							<el-input v-model="upCheckInfoForm.backupPhone" maxlength="13"></el-input>
						</el-form-item>
						<el-form-item label="司机类型" prop="driverType">
		...
		<!--司机详情信息弹窗-->
		<el-dialog class="capacity-pop capacity-pop-big" title="司机详情信息" :visible.sync="carDetailsInfo" width="" center>
		...
		<!--道路运输许可证详情信息弹窗-->
		<el-dialog class="capacity-pop capacity-pop-big" title="驾驶证详情信息" :visible.sync="dlDetailsInfo" width="55%" center>
			<div class="capacity-pop-in checkInfo">
				<el-form label-width="130px" class="demo-ruleForm">
					<div class="info-item clear">
		...
		<!--保险详情信息弹窗-->
		<el-dialog class="capacity-pop capacity-pop-big" title="从业资格证详情信息" :visible.sync="qcDetailsInfo" width="55%" center>
			<div class="capacity-pop-in checkInfo">
				<el-form label-width="130px" class="demo-ruleForm">
					<div class="info-item clear">
						<el-form-item label="姓名：">
	...
	export default {
		...
		data() {
			...
			return {
				...
				changeCarForm: {},
				changeCarRules: {},
				driveDlType: ['A1', 'A2', 'A3', 'B1', 'B2', 'C1', 'C2', 'C3'],
				driverImgs: [],
				dlFirstTimeStr: '',
				dlEndTimeStr: '',
				dlStartTimeStr: '',
				...
				searchDriverTypeOp:[{id:1,name:"大板司机"},{id:2,name:"小板司机"}],
				searchBindTypeOp:[{id:'',name:"全部"},{id:2,name:"未关联"},{id:1,name:"已关联"}],
				searchVerifyOp:[{id:-1,name:"全部"},{id:3,name:"审核中"},{id:1,name:"审核通过"},{id:2,name:"审核失败"},{id:0,name:"待提交"}],
				searchDriverType:'',
				searchBindType:'',
				searchAuthState:-1,
				searchUserName:'',
				searchDLNo:'',
				searchQcNo:'',
				searchUpdateStartTime:'',
				searchUpdateEndTime:'',
				searchVerifyEndTime:'',
				searchVerifyStartTime:'',
				upLoadFile:[],
				showError:false,
				reason:'',
				showDel:false,
		...
		methods: {
			initData(){
				...
				let searchData={
					...
					offset:this.offset,
					driverType:this.searchDriverType,
					isBindCar:this.searchBindType,
					verifyStatus:this.searchAuthState,
					userName:this.searchUserName,
					dlNo:this.searchDLNo,
					qcNo:this.searchQcNo,
					updateStartTime:this.searchUpdateStartTime,
					updateEndTime:this.searchUpdateEndTime,
					verifyEndTime:this.searchVerifyEndTime,
					verifyStartTime:this.searchVerifyStartTime,
			...
			seachBtnFun(){
                ...
                if(searchData.updateStartTime){
                    searchData.updateStartTime = this.common.formatterDate(null, null, searchData.updateStartTime) + " 00:00:00";
                }
                if(searchData.updateEndTime){
                    searchData.updateEndTime = this.common.formatterDate(null, null, searchData.updateEndTime) + " 23:59:59";
                }
				if(searchData.searchVerifyStartTime){
					searchData.searchVerifyStartTime = this.common.formatterDate(null, null, searchData.searchVerifyStartTime) + " 00:00:00";
				}
				if(searchData.searchVerifyEndTime){
					searchData.searchVerifyEndTime = this.common.formatterDate(null, null, searchData.searchVerifyEndTime) + " 23:59:59";
				}
				let data={
					'data':JSON.stringify(searchData)
				}
				this.$post("companyDriver/queryDriver",data).then(data=>{
					if (data.code != 0) {
						that.$message.error(data.msg);
					} else {
						that.tableData=data.data;
			...
			// 清空按钮
			clearSeachBtnFun(){
				this.searchDriverType="";
				this.searchBindType="";
				this.searchAuthState= -1;
				this.searchUserName="";
				this.searchDLNo="";
				this.searchQcNo="";
				this.searchUpdateStartTime="";
				this.searchUpdateEndTime="";
				this.searchVerifyStartTime="";
				this.searchVerifyEndTime="";
			},
			...
			delConfir(pkId, driverType) {
				let that = this;
				this.$confirm('此操作将该司机解雇吗？, 是否继续?', '提示', {
					...
					this.$message({type: 'info', message: '已取消解雇'});
				});
			},
			delBind() {
				let that = this;
			...
			confirmUpBtn() {
				this.$refs.upCheckInfoForm.validate((valid) => {
					if (valid) {
						...
						that.$post("agency/personalAuth", {name:this.upCheckInfoForm.userName,idCard:this.upCheckInfoForm.idCard}).then(function (data) {
							if (data.code == 0) {
								companyDriverVO['companyDriver']['realStatus'] = 2;
							} else {
								companyDriverVO['companyDriver']['realStatus'] = 1;
							...
							if (data.code == 0) {
								...
								that.$post("companyDriver/getInfoById", {driverId: that.driverPkId}).then(function (data) {
									if (data.code == 0) {
										...
										//that.upCheckInfoForm.areaId = data.resultData.companDriverVO.companyDriver.areaId;
			...
			updateInfoFun(pkId) {
				...
				this.$post("companyDriver/getInfoById", {driverId: pkId}).then(data => {
					if (data.code == 0) {
						...
						if(that.updateForm.verifyStatus == 2){
							this.showError=true;
							this.reason = data.resultData.companDriverVO.companyDriver.verifyReason;
							/*document.getElementById("errorInfo").style.display="block";

							document.getElementById("reason").innerHTML=data.resultData.companDriverVO.companyDriver.verifyReason;*/
						}else {
